...
def index(request):
    ...
    if geo_response.status_code != 200 or not geo_data:
        return render(request, "main/index.html", {
            "error": f"City '{city}' not found or API error!",
            "city": city
        })
    ...
    if air_response.status_code != 200 or "list" not in air_data or not air_data["list"]:
        return render(request, "main/index.html", {
            "error": "Error fetching air pollution data.",
            "city": city
        })
...
def air_history(request):
    ...
    if not geo_data:
        return render(request, "main/air_history.html", {"error": "City not found!", "city": city})
    ...
    if "list" not in air_data or not air_data["list"]:
        return render(request, "main/air_history.html", {"error": "No historical data found.", "city": city})
